...
describe("fetch-live pipeline integration (CWA)", () => {
  ...
  it(
    "drops provenance rows for retired sources that have no direct adapter",
    async () => {
      const dataDir = setupEnv();
      writeFileSync(
      ...
      const provenance = readJson(join(dataDir, "provenance.json"));
      expect(provenance.sources.some((source: any) => source.datasetId === "13228")).toBe(false);
    },
    60_000,
  );
...
describe("fetch-live pipeline integration (missing)", () => {
  it("writes missing-person events when missing is selected without police", async () => {
    const dataDir = setupEnv();
    process.env.SOURCES = "missing";
    process.env.EXCLUSIVE = "1";
    ...
    const domestic = readJson(join(dataDir, "domestic.json"));
    expect(domestic.filter((event: any) => event.source?.datasetId === "14420")).toHaveLength(2);
    const provenance = readJson(join(dataDir, "provenance.json"));
