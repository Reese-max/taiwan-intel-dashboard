...
export function buildNewsSourceContribution({
  ...
  const finalRows = Array.from(rows.values()).map((row) => {
    ...
    const lowContribution = lowBecauseZero || lowBecauseTiny;
    return {
      ...row,
      dedupedAway,
      nonPolice,
...
export function formatNewsSourceContributionReport(contribution, { limit = 20 } = {}) {
  ...
  const lines = [
    `台灣新聞來源貢獻（最終/原始）：${totals.finalEvents || 0}/${totals.raw || 0}；去重後 ${totals.rawUnique || 0}；警政相關 ${totals.policeRelevant || 0}`,
  ];
  ...
  for (const [idx, row] of shown.entries()) {
    ...
    lines.push(
      `${idx + 1}. ${row.label}：${row.finalEvents}/${row.raw}（去重後 ${row.rawUnique}，警政 ${row.policeRelevant}，去重掉 ${row.dedupedAway}，占比 ${pct}）${warning}`,
    );
  }
