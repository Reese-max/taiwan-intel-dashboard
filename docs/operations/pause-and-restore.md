...
- `env-secrets` 會以 `gh secret list --json name` 只檢查 GitHub Actions secrets 名稱，不讀取、輸出或寫入值；唯一必要名稱為 `CLOUDFLARE_API_TOKEN`。`DEPLOY_BASE_URL` 與 `CLOUDFLARE_ACCOUNT_ID` 都由 `update-and-deploy.yml` 明確設定，不是 secrets；演練會另外檢查 Cloudflare Pages 專案、production 分支與 `accountId`。
