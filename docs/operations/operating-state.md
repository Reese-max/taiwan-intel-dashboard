...
- `RESTORING` → `ACTIVE`：復原檢查清單通過後，把通過證據存成
  `docs/operations/receipts/restore-<UTC 日期>.md` 或 `.json`，內容須含預期 repo（`Reese-max/taiwan-intel-dashboard`）與通過結論，並把路徑寫進契約檔 `evidence.restoreReceipt`。路徑不得越界或引用無效/失敗之 run。
- → `DEGRADED`：系統在跑但未完成受控復原（例如排程被提前重新啟用）。

## 驗證指令

```bash
node scripts/operating-state.mjs validate       # 契約合法與收據完整性？
...
`guard-schedule` 被擋時輸出 `OPERATING_STATE_RECEIPT` JSON，依參數判定 exit code；
CI（`pr-check.yml` / 測試）會在每次 PR 檢查 validate + verify-docs。
